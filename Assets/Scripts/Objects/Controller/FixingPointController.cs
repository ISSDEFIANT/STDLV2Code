...
public class FixingPointController : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        if (HubSS.Hubs != null && HubSS.Hubs.Length > 0)
        {
            ...
            ExitFlag.ExitFlag = transform.position + (transform.rotation * HubSS.Hubs[0].ExitPoint)*1.5f;
        }
    }
            ...
            if (awaitingPoint.AwaitingShipsOnHub.Count > 0)
            {
                if (_h.EnteringShip != null)
                {
                    if (awaitingPoint.AwaitingShipsOnHub.Any(x => x == _h.EnteringShip) &&
                        _h.EnteringShip.captain.ToStayPoint)
                        awaitingPoint.AwaitingShipsOnHub.Remove(_h.EnteringShip);
                }

                if (_h.ShipFixing && _h.EnteringShip == null && !_h.IsConstructing)
                {
    ...
    public void DockingCall(Mobile _ship)
    {
        ...
        {
            if (HubSS.Hubs[i].ShipFixing && HubSS.Hubs[i].EnteringShip == _ship)
            {
                isEntering = true;
            }
        }
