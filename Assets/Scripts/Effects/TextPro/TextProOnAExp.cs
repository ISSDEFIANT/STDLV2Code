...
public class TextProOnAExp : TextProOnACurve
{
    ...
    /// </summary>
    [SerializeField] [Tooltip("The base of the exponential curve")]
        ...
        TMP_TextInfo textInfo, int charIdx)
    {
        ...
        Vector2 newMideBaselinePos = new Vector2(x0,
            y0 - textInfo.lineInfo[0].lineExtents.max.y *
            textInfo.characterInfo[charIdx].lineNumber); //actual new position of the character
        ...
        return Matrix4x4.TRS(new Vector3(newMideBaselinePos.x, newMideBaselinePos.y, 0),
            Quaternion.AngleAxis(Mathf.Atan(Mathf.Log(m_expBase) * y0) * Mathf.Rad2Deg, Vector3.forward), Vector3.one);
    }
}